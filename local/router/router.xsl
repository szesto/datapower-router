...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="dpconfig:Routes" select="'routes.xml'"/>
    <dp:param name="dpconfig:Routes" type="dmString" xmlns="">
        <display>Routes</display>
        <description>routes file</description>
    ...
    <xsl:variable name="category" select="$domain"/>
    
    <xsl:template match="/">
        <!-- load routes file -->
        <xsl:variable name="routes" select="document($dpconfig:Routes)/routes"/>
        ...
        <xsl:message dp:priority="error">
            uri: <xsl:value-of select="$uri"/>;;route-uri: <xsl:value-of select="$route-matches/@uri"/>;;match-count: <xsl:value-of select="$match-count"/>"/>
        </xsl:message>
        ...
        <xsl:choose>
            ...
            <xsl:when test="0 = $match-count">
                <!-- log message -->
                <xsl:message dp:priority="error">
                    Route not found. Service URI = <xsl:value-of select="$uri"/>
                </xsl:message>
            ...
            <xsl:when test="1 &lt; $match-count">
                <!-- log message -->
                <xsl:message dp:priority="error">
                    Multiple route matches. Match count = <xsl:value-of select="$match-count"/>. Service URI = <xsl:value-of select="$uri"/>
                </xsl:message>                
            </xsl:when>
            ...
            <xsl:otherwise>
                <!-- log message -->
                <xsl:message dp:priority="error">
                    Route match found. Service URI = <xsl:value-of select="$uri"/>. Destination: <xsl:value-of select="$route-matches/destination"/>
                </xsl:message>
            </xsl:otherwise>
        </xsl:choose>
        ...
        <xsl:message dp:priority="error">longest-matching-uri = <xsl:value-of select="$longest-matching-uri"/></xsl:message>
        ...
        <xsl:message dp:priority="error">routing url: <xsl:value-of select="concat($destination,$uri)"/></xsl:message>
        ...
        <xsl:if test="string-length($tls-client-profile) &gt; 0">
            <xsl:message dp:priority="error">routing url ssl profile = <xsl:value-of select="$tls-client-profile"/></xsl:message>
